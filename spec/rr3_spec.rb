...
describe Rr3 do
  ...
  it 'matches' do
    expect(@tree.match(1, "/bar").data).to be_nil
    expect(@tree.match(1, "/zoo").data).to eql 3
    expect(@tree.match(1, "/foo/bar").data).to eql 1
    expect(@tree.match(1, "/post/1234").data).to eql 2
    expect(@tree.match(1, "/user/1234").data).to eql 1
    expect(@tree.match(1, "/user/xxx").data).to eql false
